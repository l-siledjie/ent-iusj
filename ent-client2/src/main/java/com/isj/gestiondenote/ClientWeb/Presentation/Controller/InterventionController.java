...
public class InterventionController {
    ...
    public String creerIntervention(Model model, HttpSession session) {
        ...
        // Object[] intervention = restTemplate.getForObject(URL.BASE_URL_INT +
        // "soumettre/"+session.getAttribute("matricule")+"/"+ " fff ", Object[].class);
    ...
    public ModelAndView listeIntervention(Model model, HttpSession session) {
        ...
        Object[] interventions = restTemplate
                .getForObject(URL.BASE_URL_INT + "Liste/etudiant/" + session.getAttribute("matricule"), Object[].class);
    ...
    @GetMapping("/singleIntervention/{intervention}")
    public ResponseEntity<Object[]> singleIntervention(@PathVariable Long intervention, HttpSession session) {
        try {
            RestTemplate restTemplate = new RestTemplate();
            Object[] interventions = restTemplate
                    .getForObject(URL.BASE_URL_INT + "one/etudiant/" + intervention, Object[].class);

            return ResponseEntity.ok(interventions);
        } catch (Exception e) {
            // Gestion d'erreur
            e.printStackTrace(); // Affichez l'erreur dans la console (vous pouvez personnaliser cela selon vos besoins)
            return ResponseEntity.status(500).body(new Object[0]); // Réponse avec code d'erreur 500
        }
    }
    ...
    public ModelAndView listeInterventionParDepartement(Model model, HttpSession session) {
        ...
        Object[] interventions = restTemplate.getForObject(
                URL.BASE_URL_INT + "Liste/Departement/" + session.getAttribute("matricule"), Object[].class);
        model.addAttribute("interventions", interventions);
    ...
    public String prendreEnCharge(Model model, HttpSession session, @PathVariable Long idIntervention) {
        ...
        RestTemplate restTemplate = new RestTemplate();
        restTemplate.put(
                URL.BASE_URL_INT + "prendre-en-charge/" + idIntervention + "/" + session.getAttribute("matricule"),
                null);
    ...
    public ResponseEntity<?> downloadFile(Model model, HttpSession session, @PathVariable Long id) {
        ...
        Modal.model(model);
        RestTemplate restTemplate = new RestTemplate();
        // System.out.println(restTemplate.getForObject(URL.BASE_URL_INT +
        // "pieceJointe/"+id, Object.class));
